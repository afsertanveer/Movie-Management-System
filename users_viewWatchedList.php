...
<body>
  ...
  <section class="contact-us" style="height: 1420px;">
  	<?php
  	$sql = pg_query($db, "select title,genre,releasedate from film,ratingfilm where  film.f_id=ratingfilm.f_id and ratingfilm.u_id='$user_id'");
		$numofrow=pg_num_rows($sql);
		if($numofrow>0){
			?>
    <h2 style="color:white; margin-left:40%; margin-bottom: 2%; margin-top: 2%;">MY WATCHED LIST</h2>
