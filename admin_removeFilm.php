...
<body>
                ...
                <div class="row">
                    ...
                    <h2 style=" text-align: center; color:white; margin-top:10%; margin-left: 35%;">DELETE FILM LIST</h2>
                     <table  style="margin-top: 25px; width: 1700px;">
                       <tr style="background-color:#a12c2f; border:1px solid white; height: 70px;">
    ...
    {
      $title = $_POST['title'];    
      $sql = pg_query($db, "select * from film where title='$title'");
   ...
   ?>
    <?php 
        $sql = pg_query($db, "select * from film");
    ?>
    <?php 
    while($rows = pg_fetch_assoc($sql)){    
         $dfid=$rows['f_id'];
         $df_title=$rows['title'];       
    ?>
                    <tr style="vertical-align:top; border:1px solid white; height: 70px;">
                        ...
                        <td style="border:1px solid white" ><a href="deletefilm.php?dfid=<?php echo $dfid;?>">Delete</a></td>
                    </tr>
   <?php    } ?>
                  </table>
               </div>
